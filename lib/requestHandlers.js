...
requestHandlers.save = function(response, request) {
    ...
    if (request.method == 'POST') {
        ...
        request.addListener("end", function() {
            ...
            var id = POST.id;
            if (id != null) {
                article.update({_id:db.bson_serializer.ObjectID.createFromHexString(id)}, {title: POST.title, body: POST.body, date: Date()}, function(error, docs) {
...
requestHandlers.manageArticle = function(response, request) {
	...
	data.find({}, {"_id": true, "title": true}).toArray(function(error, articles) {
		if (error) {
            ...
            var items = '';
            for(var i = 0; i < articles.length; i++) {
                items += '<li><span>' + (i+1) + '. </span><a href="edit?id=' + articles[i]._id + '">' + articles[i].title +'</a></li>';
            }
